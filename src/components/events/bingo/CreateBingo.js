    ...
    async function handleSubmit(e){
        ...
        try{
            ...
            if(!comCheckSnap.exists()){
                ...
                const eventRef = await createEvent("bingo", "pending", communitiesInvolved, playersInvolved)
                console.log("Event ID we just created is: ", eventRef.id)
                const invRef = await createInvitation("open", challengerID, challengeeID, eventRef.id, "bingo")
                console.log("Invitation ID we just created is: ", invRef.id)
                await addCommunityInvitation(challengerID, invRef.id, eventRef.id, "outgoing")
                await addCommunityInvitation(challengeeID, invRef.id, eventRef.id, "incoming")
            ...
            <Card style={{display: 'flex'}}>
                <Card.Body>
                    ...
                    <Form.Group className="mb-3" controlId="communityToChallenge">
                            ...
                            <Form.Control type="text" placeholder="Enter community name" onChange={(event) => setChallengeeID((event.target.value).replaceAll(regexp, "-"))}/>
