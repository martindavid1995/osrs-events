...
export default function CreateEvent() {
        ...
        if (!auth.currentUser){
          ...
          if (comDocSnap.exists()){
            if (!(comDocSnap.data().adminUIDs).includes(auth.currentUser.uid)){
              ...
              const querySnapshot = await getDocs(collection(db, "event-information"))
              var events = []
              var descripts = []
              var thumbs = []
              var navUrls = [] 
  ...
  return (
    <>
    <Row>
      <Col className='text-center'><h3>Create A Community Event</h3></Col>
    </Row>
    <Row><Col>
    <Card style={{display: 'flex', flexDirection: 'row', flexWrap: 'wrap'}}>
      {
        supportedEvents.map((evt, index) => <EventCreationSlice key={evt+index} eventName={evt} eventDescription={descriptions[index]} eventThumbnail={thumbnailURLs[index]} navUrl={navUrl[index]} whenClicked={handleClick}/>)
      }
   </Card>
   </Col></Row>
  </>
